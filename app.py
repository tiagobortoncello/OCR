...
def correct_ocr_text(raw_text):
    ...
    - **APÓS a correção e remoção, remova todas as quebras de linha (newlines), retornos de carro e múltiplos espaços, unindo todo o texto em um único parágrafo contínuo.**
...
st.markdown("Faça o upload de um PDF digitalizado. A IA irá processar o texto, **remover o cabeçalho** e formatar o corpo em **linha contínua**.")
...
if uploaded_file is not None:
    ...
    try:
        ...
        if os.path.exists("/tmp/output.txt"):
            ...
            with st.spinner("Removendo cabeçalho, corrigindo ortografia arcaica, erros de OCR e formatando o texto em linha única com Gemini..."):
                sidecar_text_corrected = correct_ocr_text(sidecar_text_raw)

            # Exibe o texto corrigido no corpo principal em formato contínuo
            st.text_area("Texto Contínuo Corrigido e Filtrado (Gemini)", sidecar_text_corrected, height=200, key="corrected_text_final")
            
            # Adiciona botão de download para o texto
            st.download_button(
                label="⬇️ Baixar Texto Corrigido (.txt)",
                data=sidecar_text_corrected.encode('utf-8'),
                file_name="texto_corrigido_sem_cabecalho.txt",
                mime="text/plain"
            )
